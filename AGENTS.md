...
이 섹션은 Claude Code 사용 시 효율적인 작업 패턴을 정의합니다.
...
Phase 1: 사이드바 HTML 구조 생성 (토글 기능 제외)
  ...
  - 기존 콘텐츠가 정상 렌더링되는지 확인
  - 완료 후 커밋 & 다음 단계 진행

Phase 2: Stimulus 컨트롤러로 토글 기능 추가
  - app/javascript/controllers/sidebar_controller.js 생성
  - data-controller 및 data-action 연결
  - 토글 동작 테스트 (console.log로 확인)
  - 완료 후 커밋 & 다음 단계 진행

Phase 3: 반응형 스타일 및 애니메이션 추가
  - Tailwind breakpoint 클래스 적용
  - transition 및 transform 추가
  - 모바일/데스크톱 모두에서 테스트
  - 완료 후 최종 커밋
```
...
# 2단계: 모든 요청 사항을 파일별로 그룹화하여 리스트업
# (Claude가 자동으로 수행)

# 3단계: 모든 변경 사항을 순차적으로 적용
# (각 파일의 컨텍스트를 읽고 수정)

# 4단계: 테스트 실행으로 검증
bin/rails test          # 전체 테스트
bin/rubocop --autocorrect-all  # 스타일 수정
...
- Refactored <issue 3> in <file>

Co-Authored-By: Claude Sonnet 4.5 <noreply@anthropic.com>"
```

**중요:** 세션이 끊기기 전에 모든 리뷰 코멘트를 처리하고 커밋까지 완료해야 합니다. 일부만 처리하고 종료하면 어떤 코멘트가 남았는지 추적이 어렵습니다.

### 자율성과 명시적 검증의 균형

**높은 자율성이 유용한 경우:**
- 명확한 목표가 있는 리팩토링 (예: 컨트롤러 로직을 서비스로 추출)
- 패턴이 확립된 반복 작업 (예: 신규 모델에 RBS 타입 추가)
...
**원칙:** 편집 후 즉시 관련 테스트를 실행하여 회귀를 조기 발견합니다. 세션 종료 전에 전체 테스트 스위트가 통과하는지 확인해야 합니다.
