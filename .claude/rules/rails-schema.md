...
- **ra_news.posts** (18 cols) — article_id:bigint, body:text, boosters_count:integer(=0), children_count:integer(=0), depth:integer(=0), federails_actor_id:bigint, federated_url:string, lft:integer, likers_count:integer(=0), media_attachments:jsonb(=[]), parent_id:bigint, rgt:integer, slug:string, title:string, url:string, user_id:bigint | FK: ra_news.article_id→ra_news.articles, ra_news.federails_actor_id→ra_news.federails_actors, ra_news.post_id→ra_news.posts, ra_news.user_id→ra_news.users | Idx: federated_url(unique), parent_id+created_at, slug(unique)
