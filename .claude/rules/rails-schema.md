...
- **slack_article_deliveries** (10 cols) — channel_id:string, channel_name:string, error_message:text, sent_at:datetime, slack_message_ts:string, status:string(=sent) | FK: article_id→articles, slack_workspace_id→slack_workspaces | Idx: article_id+slack_workspace_id+channel_id(unique)
  status: sent, failed
- **slack_workspaces** (8 cols) — bot_access_token:string, bot_user_id:string, last_verified_at:datetime, status:string(=active), team_id:string, team_name:string | Idx: team_id(unique)
...
- **user_workspace_subscriptions** (8 cols) — active:boolean(=false), channel_id:string, channel_name:string, slack_user_id:string | FK: slack_workspace_id→slack_workspaces, user_id→users | Idx: user_id+slack_workspace_id(unique)
