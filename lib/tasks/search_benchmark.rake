...
namespace :search do
  ...
  task benchmark: :environment do
    ...
    # seed 데이터가 테스트 DB에 영구 잔류해 다른 테스트를 오염시키지 않도록
    # 전체 벤치마크를 트랜잭션으로 감싸고 마지막에 항상 롤백한다.
    ActiveRecord::Base.transaction do
      ...
      hybrid_ranked = queries.map { |q| Articles::HybridSearch.run(query: q, limit: 100) }

      require "search_benchmark"

      fts_mrr    = SearchBenchmark.mrr(fts_ranked, relevant_sets)
      hybrid_mrr = SearchBenchmark.mrr(hybrid_ranked, relevant_sets)
      ...
      raise ActiveRecord::Rollback # 데이터베이스 오염 방지를 위해 항상 롤백
    end
  end
  ...
  def stub_embeddings_for_benchmark!
    ...
    # 3072차원에서 코사인 유사도가 의미 있으려면 대부분의 차원이 0에 가까워야 한다.
  ...
  def seed_benchmark_data!
    ...
    data = [
      { tko: "[BENCH] Ruby 3.4 성능 최적화",  ten: "Ruby 3.4 Performance Optimization",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[0] = 1.0 } },
      { tko: "[BENCH] 한국어 NLP 기술 동향",   ten: "Korean NLP Technology Trends",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[1] = 1.0 } },
      { tko: "[BENCH] Rails 8 비동기 쿼리 가이드", ten: "Rails 8 Async Query Guide",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[2] = 1.0 } },
      ...
      { tko: "[BENCH] 파이썬 3.13 릴리스 노트", ten: "Python 3.13 Release Notes",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[100] = 1.0 } },
      { tko: "[BENCH] 리눅스 커널 6.10 변경사항", ten: "Linux Kernel 6.10 Changes",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[101] = 1.0 } },
      { tko: "[BENCH] React 19 서버 컴포넌트", ten: "React 19 Server Components",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[102] = 1.0 } },
      { tko: "[BENCH] PostgreSQL 17 병렬 쿼리", ten: "PostgreSQL 17 Parallel Query",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[103] = 1.0 } },
      { tko: "[BENCH] Swift 6 동시성 모델", ten: "Swift 6 Concurrency Model",
        ...
        emb: (Array.new(3072, 0.0)).tap { |v| v[104] = 1.0 } }
    ]
