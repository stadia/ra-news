...
class DiscordArticleDeliveryJobTest < ActiveJob::TestCase
  ...
  test "존재하지 않는 article_id면 RecordNotFound 에러가 발생한다" do
    assert_raises(ActiveRecord::RecordNotFound) do
      DiscordArticleDeliveryJob.new.perform(-1, @channel.id)
    end
  end

  test "존재하지 않는 channel_id면 RecordNotFound 에러가 발생한다" do
    assert_raises(ActiveRecord::RecordNotFound) do
      DiscordArticleDeliveryJob.new.perform(@article.id, -1)
    end
  end
  ...
  test "channel 필수 필드가 누락되면 ArgumentError가 발생한다" do
    ...
    assert_raises(ArgumentError) do
      DiscordArticleDeliveryJob.new.perform(@article.id, @channel.id)
    end

    # teardown
    @channel.update_columns(webhook_url: "https://discord.com/api/webhooks/EXAMPLE/REDACTED")
  end
end
