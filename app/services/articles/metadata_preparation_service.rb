...
module Articles
  class MetadataPreparationService < OperationService
    ...
    def resolve_response(article)
      response = fetch_url_content(article.url)
      return Failure(:fetch_failed) if response.nil?

      response = follow_redirection(article, response)
      return Failure(:fetch_failed) if response.nil?
    ...
    def normalize_article_url(article)
      ...
      article.url = normalized_url(parsed_url)
      article.host = parsed_url.host
      article.is_youtube = parsed_url.host&.match?(/youtube/i) == true
      article.deleted_at = Time.zone.now if should_discard_url?(article, parsed_url)
      Success(article)
    ...
    def apply_webpage_metadata(article, body)
      ...
      article.published_at = published_at_for(article, body)
    ...
    def ensure_article_slug(article)
      ...
      base = article.title.presence
      article.slug = base ? base.parameterize.presence || random_slug : random_slug
