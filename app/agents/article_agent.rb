...
<<~PROMPT
  ...
  - 비교가 필요한 경우 표 대신 bullet과 소제목으로 정리한다.
  - 문장 끝은 단정적인 보고서체로 유지한다. 예: "~함", "~임", "~제공함", "~지원함"
  ...
  - 콘텐츠 성격에 따라 구조를 자유롭게 구성한다. 단, 기능 비교가 핵심이면 비교 중심 구조를 우선한다
  - 관련 기사가 제공된 경우, 실제로 연관성이 있을 때만 마크다운 링크로 자연스럽게 언급한다
  - 제품/플랫폼 업데이트 기사에서는 아래 형식을 우선 참고한다:
    - `### 무엇이 바뀌었나`
    - `### 핵심 기능 또는 계층별 차이`
    - `### 활용 사례`
    - `### 운영 및 기술 세부 사항`
  - 필요하면 bullet을 사용해 Flex vs Priority 같은 항목을 명확히 대비한다
