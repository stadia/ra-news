module ApplicationHelper
  def time_ago_in_words_korean(time)
    return "알 수 없음" unless time

    seconds = Time.current - time

    case seconds
    when 0..59
      "방금"
    when 60..3599
      "#{(seconds / 60).to_i}분"
    when 3600..86399
      "#{(seconds / 3600).to_i}시간"
    when 86400..2591999
      "#{(seconds / 86400).to_i}일"
    when 2592000..31535999
      "#{(seconds / 2592000).to_i}개월"
    else
      "#{(seconds / 31536000).to_i}년"
    end
  end

  def responsive_image_tag(source, options = {})
    # 반응형 이미지를 위한 헬퍼
    default_options = {
      loading: "lazy",
      class: "w-full h-auto"
    }
    image_tag(source, default_options.merge(options))
  end

  def truncate_smart(text, length: 100)
    return "" unless text

    if text.length <= length
      text
    else
      text.truncate(length, omission: "...")
    end
  end

  # app/helpers/application_helper.rb
  def nav_link_to(text, path, options = {})
    options[:class] = "block py-3 px-4 text-gray-100 hover:text-white rounded-sm md:hover:text-white md:p-0 transition-colors duration-150 min-h-[44px] flex items-center".html_safe
    options[:"aria-current"] = "page" if current_page?(path)
    link_to(text, path, options)
  end

  def btn_class(variant: nil, size: nil, outline: false, extra_classes: nil)
    classes = [ "btn" ]
    classes << "btn-#{variant}" if variant
    classes << "btn-outline" if outline
    classes << "btn-#{size}" if size
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end

  def btn_link_to(text = nil, path = nil, variant: nil, size: nil, outline: false, **options, &block)
    if block_given?
      path = text
      options[:class] = [ btn_class(variant: variant, size: size, outline: outline), options[:class] ].compact.join(" ")
      link_to(path, options, &block)
    else
      options[:class] = [ btn_class(variant: variant, size: size, outline: outline), options[:class] ].compact.join(" ")
      link_to(text, path, options)
    end
  end

  def btn_button_to(text = nil, path = nil, variant: nil, size: nil, outline: false, **options, &block)
    options[:class] = [ btn_class(variant: variant, size: size, outline: outline), options[:class] ].compact.join(" ")
    button_to(text, path, options, &block)
  end

  def input_class(variant: nil, size: nil, bordered: true, extra_classes: nil)
    classes = [ "input" ]
    classes << "input-bordered" if bordered
    classes << "input-#{variant}" if variant
    classes << "input-#{size}" if size
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end

  def textarea_class(variant: nil, size: nil, bordered: true, extra_classes: nil)
    classes = [ "textarea" ]
    classes << "textarea-bordered" if bordered
    classes << "textarea-#{variant}" if variant
    classes << "textarea-#{size}" if size
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end

  def checkbox_class(variant: nil, size: nil, extra_classes: nil)
    classes = [ "checkbox" ]
    classes << "checkbox-#{variant}" if variant
    classes << "checkbox-#{size}" if size
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end

  def select_class(variant: nil, size: nil, bordered: true, extra_classes: nil)
    classes = [ "select" ]
    classes << "select-bordered" if bordered
    classes << "select-#{variant}" if variant
    classes << "select-#{size}" if size
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end

  def toggle_class(variant: nil, size: nil, extra_classes: nil)
    classes = [ "toggle" ]
    classes << "toggle-#{variant}" if variant
    classes << "toggle-#{size}" if size
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end

  def radio_class(variant: nil, size: nil, extra_classes: nil)
    classes = [ "radio" ]
    classes << "radio-#{variant}" if variant
    classes << "radio-#{size}" if size
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end

  def label_class(extra_classes: nil)
    classes = [ "label" ]
    classes << extra_classes if extra_classes.present?
    classes.join(" ")
  end
end
