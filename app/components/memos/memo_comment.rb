...
class Components::Memos::MemoComment < Components::Base
  ...
  def initialize(comment:, memo:, depth: 0, children: {})
    @comment = comment
    @memo = memo
    @depth = depth
    @children = children
  ...
  def delete_button
    if @comment.guest?
      ...
      button_to(
        memo_comment_path(@memo, @comment),
        method: :delete,
  ...
  def reply_button
    div(class: "mt-3 flex items-center justify-between text-sm") do
      render RubyUI::Button.new(
        ...
        class: "inline-flex items-center text-gray-400 hover:text-blue-400 transition-colors hover:bg-transparent"
      ) do
  ...
  def guest_delete_modal
    render Components::Memos::MemoDeleteModal.new(memo: @memo, comment: @comment)
  end

  def reply_form_section
    div(class: "bg-gray-800/50") do
      render Components::Memos::MemoCommentReplyForm.new(
        memo: @memo,
        comment: ::Comment.new,
  ...
  def children_section
    div(id: "comment_replies_#{@comment.id}", class: "ml-8 mr-3 mt-2 space-y-2 pb-3") do
      @children.each do |child, grandchildren|
        render Components::Memos::MemoComment.new(
          comment: child,
          memo: @memo,
          depth: @depth + 1,
