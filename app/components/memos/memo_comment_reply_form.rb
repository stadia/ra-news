...
class Components::Memos::MemoCommentReplyForm < Components::Base
  include Phlex::Rails::Helpers::FormWith
  include Phlex::Rails::Helpers::TurboFrameTag
  include PhlexIcons

  def initialize(memo:, comment:, parent_comment:, visible: false)
    @memo = memo
    @comment = comment
    ...
    div(class: "grid gap-3 sm:grid-cols-2") do
      div(class: "space-y-2") do
        ...
        f.text_field :guest_name,
          class: "w-full px-3 py-2 rounded-lg border bg-gray-700 text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm #{@comment.errors[:guest_name].none? ? 'border-gray-600' : 'border-red-500'}",
      ...
      div(class: "space-y-2") do
        ...
        f.password_field :guest_password,
          class: "w-full px-3 py-2 rounded-lg border bg-gray-700 text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm #{@comment.errors[:guest_password].none? ? 'border-gray-600' : 'border-red-500'}",
  ...
  def body_field(f)
    div(class: "space-y-2") do
      f.text_area :body,
        rows: 3,
        class: "w-full px-4 py-2 rounded-lg border bg-gray-700 text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none text-sm #{@comment.errors[:body].none? ? 'border-gray-600' : 'border-red-500'}",
  ...
  def action_buttons(f)
    div(class: "flex items-center justify-end gap-2") do
      ...
      f.submit "답글 작성",
        class: "inline-flex items-center px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-md transition-colors"
    end
  end
end
