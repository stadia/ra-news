...
class Components::Comments::DeleteModal < Components::Base
  ...
  def view_template
    div(id: "delete_comment_modal_#{@comment.id}",
      ...
      # Background backdrop
      div(class: "fixed inset-0 bg-app/75 transition-opacity", data: { action: "click->modal#close" }) do
      end
      div(class: "flex min-h-full items-center justify-center p-4 text-center sm:p-0") do
        div(class: "relative transform overflow-hidden rounded-lg bg-surface text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-border-strong") do
          # Modal header
          div(class: "bg-surface px-4 py-3 sm:px-6 border-b border-border-strong") do
            h3(class: "text-base font-semibold leading-6 text-content", id: "modal-title") do
              Hero::ShieldExclamation(variant: :outline, class: "w-5 h-5 inline mr-2 text-warning")
              plain "댓글 삭제 확인"
            end
          end
          # Modal body
          div(class: "px-4 py-5 sm:p-6") do
            div(class: "mb-4") do
              p(class: "text-sm text-content-secondary mb-2") do
                "게스트 댓글을 삭제하려면 작성 시 입력한 비밀번호를 입력해주세요."
              end
              p(class: "text-sm text-content-muted") do
                        ...
                        method: :delete, local: false, class: "space-y-4" do |f|
                          render RubyUI::FormField.new do
                            ...
                            f.password_field :password,
                                                        required: true,
                                                        class:
                                                          "w-full px-3 py-2 rounded-lg border border-border-muted bg-surface-muted text-content placeholder:text-content-muted focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent transition-colors duration-200",
