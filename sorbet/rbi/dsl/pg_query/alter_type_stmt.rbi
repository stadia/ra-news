# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for dynamic methods in `PgQuery::AlterTypeStmt`.
# Please instead update this file by running `bin/tapioca dsl PgQuery::AlterTypeStmt`.


class PgQuery::AlterTypeStmt < Google::Protobuf::AbstractMessage
  sig { params(options: T.nilable(T::Array[PgQuery::Node]), type_name: T.nilable(T::Array[PgQuery::Node])).void }
  def initialize(options: T.unsafe(nil), type_name: T.unsafe(nil)); end

  sig { void }
  def clear_options; end

  sig { void }
  def clear_type_name; end

  sig { returns(Google::Protobuf::RepeatedField[PgQuery::Node]) }
  def options; end

  sig { params(value: Google::Protobuf::RepeatedField[PgQuery::Node]).void }
  def options=(value); end

  sig { returns(Google::Protobuf::RepeatedField[PgQuery::Node]) }
  def type_name; end

  sig { params(value: Google::Protobuf::RepeatedField[PgQuery::Node]).void }
  def type_name=(value); end
end
