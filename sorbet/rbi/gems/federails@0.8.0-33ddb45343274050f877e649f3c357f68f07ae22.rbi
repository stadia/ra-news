...
#
# 단, 재생성 후에는 아래 두 가지를 매번 다시 손봐야 한다(tapioca 결함/부정확):
#  1. `*rest`·`**keyrest`·`&block` 파라미터의 타입이 빈 칸으로 나온다
#     (`params(_arg0: )`). Sorbet 파서가 srb.help/5004 로 거부하므로
#     `T.untyped` 를 채워 넣는다. 파일이 `typed: false` 라도 파스 에러는 뜬다.
#  2. `Actor.find_by_account` 의 반환 타입이 non-nilable 로 잡힌다. 실제로는
#     `find_by(...) || Webfinger.fetch_actor(...)` 라 nil 이 나올 수 있고,
#     그대로 두면 ActorsController#lookup 의 nil 가드가 unreachable(7006)이 된다.
...
class Federails::Actor < ::Federails::ApplicationRecord
  ...
  class << self
    ...
    sig { params(account: ::String).returns(T.nilable(::Federails::Actor)) }
