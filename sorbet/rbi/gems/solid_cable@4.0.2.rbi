# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `solid_cable` gem.
# Please instead update this file by running `bin/tapioca gem solid_cable`.


# pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:8
module ActionCable; end

# pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:9
module ActionCable::SubscriptionAdapter; end

# pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:10
class ActionCable::SubscriptionAdapter::SolidCable < ::ActionCable::SubscriptionAdapter::Base
  include ::ActionCable::SubscriptionAdapter::ChannelPrefix

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:13
  def initialize(*_arg0); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:25
  def broadcast(channel, payload); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:39
  def shutdown(*_arg0, **_arg1, &_arg2); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:31
  def subscribe(channel, callback, success_callback = T.unsafe(nil)); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:35
  def unsubscribe(channel, callback); end

  private

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:42
  def listener; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:48
  def pubsub_executor; end
end

# pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:57
class ActionCable::SubscriptionAdapter::SolidCable::Listener < ::ActionCable::SubscriptionAdapter::SubscriberMap
  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:67
  def initialize(adapter, executor); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:124
  def add_channel(channel, on_success); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:108
  def interruptible; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:133
  def invoke_callback(*_arg0); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:92
  def listen; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:65
  def logger(*_arg0, **_arg1, &_arg2); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:129
  def remove_channel(channel); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:115
  def shutdown; end

  private

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:149
  def broadcast_messages; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:145
  def channels; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:138
  def executor; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:139
  def last_id; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:139
  def last_id=(_arg0); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:141
  def last_message_id; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:139
  def reconnect_attempt; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:139
  def reconnect_attempt=(_arg0); end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:178
  def reconnect_attempts; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:182
  def retry_connecting?; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:138
  def thread; end

  # pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:170
  def with_polling_volume; end
end

# pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:58
ActionCable::SubscriptionAdapter::SolidCable::Listener::CONNECTION_ERRORS = T.let(T.unsafe(nil), Array)

# pkg:gem/solid_cable#lib/action_cable/subscription_adapter/solid_cable.rb:63
class ActionCable::SubscriptionAdapter::SolidCable::Listener::Stop < ::Exception; end

# pkg:gem/solid_cable#lib/solid_cable/version.rb:3
module SolidCable
  class << self
    # pkg:gem/solid_cable#lib/solid_cable.rb:25
    def autotrim?; end

    # pkg:gem/solid_cable#lib/solid_cable.rb:9
    def connects_to; end

    # pkg:gem/solid_cable#lib/solid_cable.rb:21
    def message_retention; end

    # pkg:gem/solid_cable#lib/solid_cable.rb:17
    def polling_interval; end

    # pkg:gem/solid_cable#lib/solid_cable/engine.rb:5
    def railtie_helpers_paths; end

    # pkg:gem/solid_cable#lib/solid_cable/engine.rb:5
    def railtie_namespace; end

    # pkg:gem/solid_cable#lib/solid_cable/engine.rb:5
    def railtie_routes_url_helpers(include_path_helpers = T.unsafe(nil)); end

    # pkg:gem/solid_cable#lib/solid_cable.rb:51
    def reconnect_attempts; end

    # pkg:gem/solid_cable#lib/solid_cable.rb:13
    def silence_polling?; end

    # pkg:gem/solid_cable#lib/solid_cable/engine.rb:5
    def table_name_prefix; end

    # pkg:gem/solid_cable#lib/solid_cable.rb:29
    def trim_batch_size; end

    # For every write that we do, we attempt to delete trim_chance times as
    # many records. This ensures there is downward pressure on the cache size
    # while there is valid data to delete. Read this as 'every time the trim job
    # runs theres a trim_multiplier chance this trims'. Adjust number to make it
    # more or less likely to trim. Only works like this if trim_batch_size is
    # 100
    #
    # pkg:gem/solid_cable#lib/solid_cable.rb:47
    def trim_chance; end

    # pkg:gem/solid_cable#lib/solid_cable/engine.rb:5
    def use_relative_model_naming?; end

    # pkg:gem/solid_cable#lib/solid_cable.rb:37
    def use_skip_locked; end

    private

    # pkg:gem/solid_cable#lib/solid_cable.rb:58
    def cable_config; end

    # pkg:gem/solid_cable#lib/solid_cable.rb:62
    def parse_duration(duration, default:); end
  end
end

# pkg:gem/solid_cable#lib/solid_cable/engine.rb:4
class SolidCable::Engine < ::Rails::Engine; end

class SolidCable::Message < ::SolidCable::Record
  include ::SolidCable::Message::GeneratedAttributeMethods
  include ::SolidCable::Message::GeneratedAssociationMethods

  class << self
    def broadcast(channel, payload); end
    def broadcastable(*args, **_arg1); end
    def channel_hash_for(channel); end
    def channel_hashes_for(channels); end
    def trimmable(*args, **_arg1); end

    private

    def __class_attr__validators; end
    def __class_attr__validators=(new_value); end
    def __class_attr_defined_enums; end
    def __class_attr_defined_enums=(new_value); end
  end
end

module SolidCable::Message::GeneratedAssociationMethods; end
module SolidCable::Message::GeneratedAttributeMethods; end

class SolidCable::Record < ::ActiveRecord::Base
  include ::SolidCable::Record::GeneratedAttributeMethods
  include ::SolidCable::Record::GeneratedAssociationMethods

  class << self
    def non_blocking_lock; end

    private

    def __class_attr__validators; end
    def __class_attr__validators=(new_value); end
    def __class_attr_defined_enums; end
    def __class_attr_defined_enums=(new_value); end
  end
end

module SolidCable::Record::GeneratedAssociationMethods; end
module SolidCable::Record::GeneratedAttributeMethods; end

class SolidCable::TrimJob < ::ActiveJob::Base
  def perform; end

  private

  def trim?; end
  def trim_batch_size; end
end

# pkg:gem/solid_cable#lib/solid_cable/version.rb:4
SolidCable::VERSION = T.let(T.unsafe(nil), String)
